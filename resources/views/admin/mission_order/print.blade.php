...
<head>
    ...
    <title>Ordre de Mission - {{ $missionOrder->id }}</title>
    <style>
        @page {
            margin-top: 0.5cm;
            margin-bottom: 0.5cm;
...
<body>
    ...
    <div class="title-section">
        <div class="main-title">
            ORDRE DE MISSION @if($missionOrder->permanent)PERMANENT @endif POUR ANNEE {{ date('Y', strtotime($missionOrder->getStart())) }}
        </div>
    ...
    <div class="article-section">
        ...
        <div class="article-content">
            Il est prescrit à Mr : <strong>{{ $missionOrder->driver->full_name ?? 'N/A' }}</strong>, de se rendre à toutes les directions moyennant la voiture de service <strong>{{ $missionOrder->vehicule->brand ?? 'N/A' }}</strong> immatriculée sous N° <strong>{{ $missionOrder->vehicule->matricule ?? 'N/A' }}</strong>.
        </div>
    </div>
